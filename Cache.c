...
int readmem(int addr){//overwirte problem from above address addr
...
//0		printf("cold miss is working\n");
	...
	{	
		...
		{
			if(cache[index].dirty == 1)// drain_the_cacheline
			{
				for(int i=0;i<16; i++){
					memory[(drain_addr/4)+i] = cache[index].data[i];
				}//for close drain memory insert
			}//if dirty drain close
		
		
			for (int i=0; i<16; i++)//fill_the_cacheline	
		{		cache[index].data[i]=memory[(dest_addr/4)+i];}//dest_addr
		...
		else if (cache[index].tag == tag)//hit case
		{
		hitcount++;
		return cache[index].data[offset/4];
//0		printf("lw hit\n");
...
//0		printf("cold miss is working\n");
	...
	else if(cache[index].valid == 1)
	{
		if(cache[index].tag != tag)//miss
		{
			...
			for(int i=0; i<16; i++){//filling the cacheline
				cache[index].data[i]=memory[(dest_addr/4)+i];
			}//for close
		cache[index].data[offset/4]= value;
		cache[index].tag=tag;
...
//0		printf("JUMP pc : %d",idex_latch[0].jumpaddr);	
		nexe++;
		njump++;
		}
...
//0		printf("DATA DEPENDENCY WORKING distance (1) r[rs]\n");
	}
...
//0			printf("DATA DEPENDENCY WORKING distance (2) LORD WORD r[rs]\n");
			}  
        else
			{
			idex_latch[1].v1 = memwb_latch[0].temp;
//0			printf("DATA DEPENDENCY WORKING distance (2)  r[rs]\n");
			}
	}
...
//0		printf("DATA DEPENDENCY WORKING distance (1) rt\n");
	}
...
//0			printf("DATA DEPENDENCY WORKING distance (2) LORD WORD r[rt]\n");
		}
	else
		{
			idex_latch[1].v2 = memwb_latch[0].temp;
//0			printf("DATA DEPENDENCY WORKING distance (2) r[rt]\n");
		}
	}
...
//0				printf("add execution\n");
				nexe++;
				break;
			case addu:
				exmem_latch[0].temp= v1+v2;
//0				printf("v1 %d vv2 %d  exmem_latch[0].temp %d \n", v1, v2, exmem_latch[0].temp);
//0				printf("addu execution \n");
				nexe++;
				break;
			case and: 
				exmem_latch[0].temp= v1&v2;
//0				printf("and execution \n");
				nexe++;
				break;
			case jr:
				pc=v1;
...
//0				printf("jr 작동한다 pc: %d\n",pc);
				break;
			case nor :
				exmem_latch[0].temp!=(v1|v2);
//0				printf("nor execution \n");
				nexe++;
				break;
			case or:
				exmem_latch[0].temp=(v1|v2);
//0				printf("or execution \n");
				nexe++;
				break;
			case slt:
				exmem_latch[0].temp=((v1<v2) ? 1:0);
//0				printf("slt execution \n");
				nexe++;
				break;
			case sltu:
				exmem_latch[0].temp=((v1<v2) ? 1:0);
//0				printf("sltu execution \n");
				nexe++;
				break;
			case sll:
			...
			{	
//0				printf("nop instruction execution \n");
			}	
				break;
			case srl:
				exmem_latch[0].temp= v2 >> idex_latch[1].shamt;
//0				printf("srl execution \n");
				nexe++;
				break;
			case sub:
				exmem_latch[0].temp= v1 - v2;
//0				printf("sub execution \n");
				nexe++;
				break;
			case subu:
				exmem_latch[0].temp = v1 - v2;
//0				printf("subu execution \n");
				nexe++;
				break;
			default:
				break;
...
//0				printf("addi execution\n");
				nexe++;
				break;
			case sw:
				exmem_latch[0].temp=v2;
				exmem_latch[0].index= v1+ idex_latch[1].signextimm;
//0				printf("SW execution,v1 %d  signextimm : %d\n",v1, idex_latch[1].signextimm);
				nexe++;
...
//				printf("==========\n\nlw test : %d\n\n\n", exmem_latch[0].temp);
//0				printf("LW execution,v1 %d  signextimm : %d\n",v1, idex_latch[1].signextimm);
				nexe++;
				break;
			case addiu:
				exmem_latch[0].temp=v1+idex_latch[1].signextimm;
//0				printf("addiu execution \n");
				nexe++;
				break;
			case andi:
				exmem_latch[0].temp=v1 & idex_latch[1].zeroextimm;
//0				printf("lui execution \n");
				nexe++;
				break;
			case lui:
...
//0				printf("lui execution \n");
			...
			case ori:
				exmem_latch[0].temp= v1 | idex_latch[1].zeroextimm;
//0				printf("ori execution \n");
				nexe++;
				break;
			case slti:
				exmem_latch[0].temp=(v1 < (idex_latch[1].signextimm) ? 1:0);
//0				printf("slti execution \n");
				nexe++;
				break;
			case sltiu:
				exmem_latch[0].temp=((v1 < idex_latch[1].signextimm)? 1:0);
//0				printf("sltiu execution \n");
				nexe++;
				break;
			default:
				break;
...
//0					printf("BEQ else pc : %d npc %d  branchaddr : %x \n", pc,idex_latch[1].pc, idex_latch[1].branchaddr);
				}
			break;
			case bne:
				if(v1 != v2)
				{
					...
					memset(&ifid_latch[0],0,sizeof(ifid));
					memset(&idex_latch[0],0,sizeof(idex));
//0					printf("BNE right pc : %d npc %d  branchaddr : %x \n", pc,idex_latch[1].pc, idex_latch[1].branchaddr);
					nexe++;
					nbranch++;
				...
				{
				//	pc= idex_latch[1].pc+4;
//0					printf("BNE else pc : %d npc %d  branchaddr : %x \n", pc,idex_latch[1].pc, idex_latch[1].branchaddr);
					nexe++;
					nnotbranch++;
				}
...
//updating next using latch//exemlatch[0]=idex_latch[1]
	...
	exmem_latch[0].v2= v2;
//	exmem_latch[0].index=idex_latch[1].index;
//	exmem_latch[0].word=idex_latch[1].word;
...
//		memory[exmem_latch[1].word/4]=exmem_latch[1].v2;
		writemem(exmem_latch[1].index, exmem_latch[1].temp);
		writememorycount++;
//0		printf("MEMORYACCES \nSSSWWW 가 잘 진행되고 있음 \n");
//0		printf("memory[0x%08x]= 0x%08x \n", exmem_latch[1].index/4, exmem_latch[1].v2);
		nmem++;
	}
...
//		exmem_latch[1].memresult=memory[exmem_latch[1].word/4];
		memwb_latch[0].memresult = readmem(exmem_latch[1].word);
		readmemorycount++;
//0		printf("MEMORYACCESS \nLLLWWW 가 잘 진행하고 있음 \n");
//0		printf(" 0x%08x=memory[0x%08x]\n",exmem_latch[1].memresult,exmem_latch[1].word/4);
		nmem++;
	}
...
//	idex_latch[0].v1=r[idex_latch[0].rs];
	...
	hitrate=(double)(hitcount)/(hitcount+misscount)*100;
	missrate=(double)(misscount)/(hitcount + misscount)*100;
